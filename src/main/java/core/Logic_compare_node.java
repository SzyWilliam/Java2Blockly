...
public class Logic_compare_node implements Block_node {
    ...
    private Logic_compare_node(Block_node val_A, Block_node val_B, String op) {
        this.val_A = val_A;
        this.val_B = val_B;
        this.op = op;
    }
    ...
    public static Logic_compare_node logic_compare_node_factory(Node node) {
        ...
        validTypeSet.add("logic_compare");
        validTypeSet.add("text");
        ...
        if (nodeNum == 2) { // 只有运算符
            return new Logic_compare_node(null, null, op);
            ...
            if (type.equals("A")) { // 左操作数
                return new Logic_compare_node(Parser.parseNodeToBlockNode(currChild.getChildNodes().item(1), validTypeSet), null, op);
            } else { // 右操作数
                return new Logic_compare_node(null, Parser.parseNodeToBlockNode(currChild.getChildNodes().item(1), validTypeSet), op);
            }
        } else { // 两个操作数
            Node val_A = nodeList.item(3).getChildNodes().item(1);
            Node val_B = nodeList.item(5).getChildNodes().item(1);
            return new Logic_compare_node(Parser.parseNodeToBlockNode(val_A, validTypeSet), Parser.parseNodeToBlockNode(val_B, validTypeSet), op);
        }
    }
    ...
    public String toJavaCode() {
        ...
        boolean aIsString = val_A instanceof Text_node;
        boolean bIsString = val_B instanceof Text_node;
    ...
    public String toXML() {
        ...
        return "<block type=\"logic_compare\" id=\"\" x=\"\" y=\"\">" +
